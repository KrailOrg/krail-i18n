...
class DefaultTranslateTest extends Specification {
    ...
    def setup() {
        ...
        patternSource.retrievePattern(TestLabelKey.Cancel, Locale.UK) >> "Cancel"
        patternSource.retrievePattern(TestLabelKey.Ok, Locale.UK) >> "Ok"
        patternSource.retrievePattern(TestLabelKey.Cancel, Locale.GERMANY) >> "Stornieren"
        patternSource.retrievePattern(TestLabelKey.Ok, Locale.GERMANY) >> "OK"
        patternSource.retrievePattern(TestLabelKey.Ok, Locale.FRANCE) >> "Ok"
        patternSource.retrievePattern(TestLabelKey.Cancel, germanSwitzerland) >> "Stornieren"
        patternSource.retrievePattern(TestLabelKey.Ok, germanSwitzerland) >> "OK"
        ...
        patternSource.retrievePattern(TestLabelKey.Log_In, Locale.UK) >> "Log In"
        patternSource.retrievePattern(TestLabelKey.Log_In, Locale.GERMANY) >> "Einloggen"
    }
    ...
    def "translate from"() {
        ...
        expect:
        translate.from(TestLabelKey.Cancel).equals("Cancel")
        translate.from(TestLabelKey.Ok).equals("Ok")

        translate.from(TestLabelKey.Cancel, Locale.GERMANY).equals("Stornieren")
        // OK is not redefined in _de
        translate.from(TestLabelKey.Ok, Locale.GERMANY).equals("OK")

        // this in inherited from Labels_de
        translate.from(TestLabelKey.Cancel, germanSwitzerland).equals("Stornieren")
        // this is inherited from PatternLabels (2 levels of inheritance)
        translate.from(TestLabelKey.Ok, germanSwitzerland).equals("OK")
    }
    ...
    def "attempt to translate an unsupported locale should throw exception"() {
        ...
        when:
        translate.from(TestLabelKey.Ok, Locale.FRANCE)
    ...
    def "when pattern contains an I18NKey, that key should also be translated"() {
        ...
        String translation = translate.from(TestLabelKey.pattern_with_embedded_key, TestLabelKey.Log_In)
        then:
        ...
        translation = translate.from(TestLabelKey.pattern_with_embedded_key, TestLabelKey.Log_In)
    ...
    def "disabled supported locales check allows any Locale to be processed"() {
        expect:
        translate.from(false, TestLabelKey.Ok, Locale.FRANCE).equals("Ok")
    }

    def "Explicitly enabled or disabled supported locales check throws exception correctly"() {

        when:
        translate.from(false, TestLabelKey.Ok, Locale.FRANCE)
        ...
        when:
        translate.from(true, TestLabelKey.Ok, Locale.FRANCE)
